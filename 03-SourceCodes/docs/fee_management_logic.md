...
1. **[Fee](file:///d:/code-merge/03-SourceCodes/backend/src/main/java/com/cnpm/apartment/model/Fee.java)** (Khoản Phí)
...
2. **[AssignedFee](file:///d:/code-merge/03-SourceCodes/backend/src/main/java/com/cnpm/apartment/model/AssignedFee.java)** (Phí Đã Gán)
...
3. **[Receipt](file:///d:/code-merge/03-SourceCodes/backend/src/main/java/com/cnpm/apartment/model/Receipt.java)** (Biên Lai Thanh Toán)
...
4. **[CollectionPeriod](file:///d:/code-merge/03-SourceCodes/backend/src/main/java/com/cnpm/apartment/model/CollectionPeriod.java)** (Đợt Thu Phí)
   * Quản lý trạng thái đợt thu (`OPEN` - Đang mở, `CLOSED` - Đã đóng).

5. **[Household](file:///d:/code-merge/03-SourceCodes/backend/src/main/java/com/cnpm/apartment/model/Household.java)** (Hộ Gia Đình)
   * Cung cấp các chỉ số tính phí: diện tích (`area`), số nhân khẩu (`membersCount`), số xe máy (`motorcycleCount`), số ô tô (`carCount`).

6. **[UtilityRecord](file:///d:/code-merge/03-SourceCodes/backend/src/main/java/com/cnpm/apartment/model/UtilityRecord.java)** (Chỉ Số Điện Nước)
...
Để đảm bảo tính linh hoạt khi mở rộng các phương pháp tính phí mới, hệ thống áp dụng **Strategy Pattern** tại Backend thông qua interface **[FeeCalculator](file:///d:/code-merge/03-SourceCodes/backend/src/main/java/com/cnpm/apartment/service/calculator/FeeCalculator.java)** và **[CalculatorFactory](file:///d:/code-merge/03-SourceCodes/backend/src/main/java/com/cnpm/apartment/service/calculator/CalculatorFactory.java)**.
...
| :--- | :--- | :--- |
| **`FIXED`** | [FixedCalculator](file:///d:/code-merge/03-SourceCodes/backend/src/main/java/com/cnpm/apartment/service/calculator/FixedCalculator.java) | • Nếu là nợ cũ (`FEE_DEBT`): `Đơn giá (1) * Số tiền nợ cũ (quantity)`<br>• Ngược lại: Bằng đơn giá cố định `price`. |
| **`PER_PERSON`** | [PerPersonCalculator](file:///d:/code-merge/03-SourceCodes/backend/src/main/java/com/cnpm/apartment/service/calculator/PerPersonCalculator.java) | `Đơn giá * Số nhân khẩu của hộ gia đình` |
| **`PER_M2`** | [PerM2Calculator](file:///d:/code-merge/03-SourceCodes/backend/src/main/java/com/cnpm/apartment/service/calculator/PerM2Calculator.java) | `Đơn giá * Diện tích căn hộ (m²)` |
| **`PER_MOTORCYCLE`** | [PerMotorcycleCalculator](file:///d:/code-merge/03-SourceCodes/backend/src/main/java/com/cnpm/apartment/service/calculator/PerMotorcycleCalculator.java) | `Đơn giá * Số lượng xe máy của hộ gia đình` |
| **`PER_CAR`** | [PerCarCalculator](file:///d:/code-merge/03-SourceCodes/backend/src/main/java/com/cnpm/apartment/service/calculator/PerCarCalculator.java) | `Đơn giá * Số lượng ô tô của hộ gia đình` |
| **`PER_VEHICLE`** | [PerVehicleCalculator](file:///d:/code-merge/03-SourceCodes/backend/src/main/java/com/cnpm/apartment/service/calculator/PerVehicleCalculator.java) | `Đơn giá * Số lượng phương tiện (chung)` |
| **`CONSUMPTION`** | [ConsumptionCalculator](file:///d:/code-merge/03-SourceCodes/backend/src/main/java/com/cnpm/apartment/service/calculator/ConsumptionCalculator.java) | • Tự động nhận diện loại điện/nước qua từ khóa tên phí (chứa chữ "water" -> `WATER`, ngược lại -> `ELECTRICITY`).<br>• Công thức: `Đơn giá * (Chỉ số mới - Chỉ số cũ)` lấy từ bảng `UtilityRecord`. |

---

## III. Nghiệp Vụ Cốt Lõi Tại Backend ([PaymentService.java](file:///d:/code-merge/03-SourceCodes/backend/src/main/java/com/cnpm/apartment/service/PaymentService.java))
...
1. **Tra Cứu Lịch Sử ([ReceiptService.java](file:///d:/code-merge/03-SourceCodes/backend/src/main/java/com/cnpm/apartment/service/ReceiptService.java))**
   * Cho phép lấy danh sách biên lai phân trang, lọc động theo hộ gia đình hoặc theo khoảng thời gian giao dịch (`from` - `to`).

2. **Thống Kê Doanh Thu ([StatisticsService.java](file:///d:/code-merge/03-SourceCodes/backend/src/main/java/com/cnpm/apartment/service/StatisticsService.java))**
...
3. **Xuất Báo Cáo Excel ([ExportService.java](file:///d:/code-merge/03-SourceCodes/backend/src/main/java/com/cnpm/apartment/service/ExportService.java))**
...
1. **API Client ([api.js](file:///d:/code-merge/03-SourceCodes/frontend/js/api.js))**
...
2. **Giao Diện & Engine Phụ Trợ ([payment.js](file:///d:/code-merge/03-SourceCodes/frontend/js/components/payment.js))**
   * **[PaymentEngine](file:///d:/code-merge/03-SourceCodes/frontend/js/components/payment.js#L34)**: Triển khai logic tính phí (`calcAmount`) và ghi nhận nộp tiền tương đương với Backend để phục vụ riêng cho chế độ **Fallback Mode (LocalStorage)** khi không có kết nối mạng.
