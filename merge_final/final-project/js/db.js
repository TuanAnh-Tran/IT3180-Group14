...
export class ApartmentDB {
  ...
  static async init() {
    ...
    if (localStorage.getItem(STORAGE_KEYS.INITIALIZED)) {
      // Đảm bảo tài khoản accountant tồn tại trong danh sách tài khoản đã khởi tạo trước đó
      try {
        const users = JSON.parse(localStorage.getItem(STORAGE_KEYS.USERS)) || [];
        const hasAccountant = users.some(u => u.username.toLowerCase() === 'accountant');
        if (!hasAccountant) {
          users.push({
            username: 'accountant',
        ...
        console.error('Error auto-seeding accountant:', e);
      }
      return;
    }
