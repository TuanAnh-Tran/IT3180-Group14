...
function fmSeed() {
  const fees = [
    ...
    { id: uid('FEE'), name: 'Phí vệ sinh',            type: 'COMPULSORY', calcMethod: 'PER_MEMBER',  price: 72000 },
    { id: uid('FEE'), name: 'Ủng hộ ngày thương binh- liệt sỹ 27/07', type: 'VOLUNTARY', calcMethod: 'DONATION', price: 1 },
    { id: uid('FEE'), name: 'Ủng hộ ngày tết thiếu nhi',               type: 'VOLUNTARY', calcMethod: 'DONATION', price: 1 },
    { id: uid('FEE'), name: 'Ủng hộ vì người nghèo',                   type: 'VOLUNTARY', calcMethod: 'DONATION', price: 1 }
  ];
...
export class FeeManagerView {
  static render(container, currentUser, showToast) {
    ...
    container.innerHTML = `
      ...
      <div class="sf-wrap">
        <div class="chart-card" style="margin-bottom:20px;padding:16px 20px;display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:12px;">
          ...
          ${!isResident ? '<button class="btn btn-secondary" id="sf-reset-btn" style="font-size:12px;padding:8px 14px;">Reset dữ liệu mẫu</button>' : ''}
        </div>
        ...
        <div class="sf-tabs">
          ${!isResident ? '<button class="sf-tab active" data-sf="sf-db">Bảng Điều Khiển</button>' : ''}
          <button class="sf-tab ${isResident ? 'active' : ''}" data-sf="sf-fees">Khoản Thu</button>
        ...
        <div class="sf-panel ${isResident ? 'active' : ''}" id="sf-fees">
          <div class="sf-card">
            <div class="sf-row"><h3 style="margin:0;">Danh sách Khoản Thu</h3>${!isResident ? '<button class="sf-btn pri" id="sf-add-fee-btn">+ Tạo Khoản Thu Mới</button>' : ''}</div>
            <div class="sf-tbl-wrap">
              <table class="sf-tbl">
                <thead><tr><th>Mã</th><th>Tên Khoản Thu</th><th>Loại</th><th>Cách Tính</th><th>Đơn Giá</th>${!isResident ? '<th style="text-align:right;">Hành Động</th>' : ''}</tr></thead>
        ...
        <div class="sf-panel" id="sf-hh">
          <div class="sf-card">
            <div class="sf-row">
              ...
              ${!isResident ? '<button class="sf-btn sec" id="sf-add-hh-btn">+ Thêm Hộ Dân</button>' : ''}
            </div>
            <div class="sf-tbl-wrap">
              <table class="sf-tbl">
          ...
          <div class="sf-mb">
            <form class="sf-form" id="sf-fee-form">
              ...
              <div class="sf-2field">
                ...
                <div class="sf-field"><label>Cách tính</label><select id="sf-fee-calc"><option value="FIXED">Cố định</option><option value="PER_MEMBER">Theo nhân khẩu</option><option value="PER_AREA">Theo diện tích</option><option value="CONSUMPTION">Theo tiêu thụ</option><option value="DONATION">Đóng góp (Tùy tâm)</option></select></div>
              </div>
    ...
    /* ===== fees tab ===== */
    function renderFees() {
      const calcMap = { FIXED:'Cố định', PER_MEMBER:'Theo nhân khẩu', PER_AREA:'Theo diện tích', CONSUMPTION:'Theo tiêu thụ', DONATION:'Đóng góp (Tùy tâm)' };
    ...
    function renderPeriods() {
      ...
      }).join('') || `<tr><td colspan="${showActions ? 4 : 3}" style="text-align:center;color:var(--text-muted);padding:20px;">Chưa có đợt thu nào.</td></tr>`;
    ...
    function openBillModal(hhId) {
      ...
      const calcLbl = { FIXED:'Cố định', PER_MEMBER:'Nhân khẩu × Đơn giá', PER_AREA:'Diện tích × Đơn giá', CONSUMPTION:'Tiêu thụ × Đơn giá', DONATION:'Đóng góp tự nguyện' };
      ...
      q('#sf-bill-tbody').innerHTML = bill.items.map(item => {
        ...
        const actions = item.status==='UNPAID'
          ? `<button class="sf-btn suc sf-pay" data-id="${item.assignedFeeId}">Đóng tiền</button>
             ${!isResident ? `<button class="sf-btn dan sf-unassign" data-fid="${item.feeId}" data-hhid="${hhId}">Hủy gán</button>` : ''}`
          : `${!isResident ? `<button class="sf-btn sec sf-unpay" data-id="${item.assignedFeeId}">Hoàn tác</button>` : ''}`;
        return `<tr>
