 ...
 * Interacts exclusively with Spring Boot backend APIs and MySQL database.
 */

import { API } from '../api.js?v=5';

/* ─────────────────────────────────────────────
   1. FM BRIDGE — expose _getDB / _saveDB vào FM
   ───────────────────────────────────────────── */
...
export class PaymentView {
  static async render(container, currentUser, showToast) {
    ...
    // Synchronize local database with MySQL Backend
    try {
      await FM.syncWithBackend();
    } catch (e) {
    ...
    async function renderUnpaid() {
      ...
      try {
        ...
        showToast(e.message || "Failed to load unpaid fees", "error");
      }
      ...
      q('#pv-pay-tbody').innerHTML = afs.map(af => {
        const fee = { name: af.feeName, price: af.unitPrice };
        const hh  = { id: af.householdId, ownerName: af.ownerName };
        const per = { name: af.periodName };
        const amt = af.amountRequired;
    ...
    async function renderReceipts() {
      ...
      try {
        ...
        showToast(e.message || "Failed to load receipt history", "error");
      }
      ...
      q('#pv-rec-tbody').innerHTML = receipts.map(r => {
        ...
        const hhName = r.ownerName;
        const feeName = r.feeName;
    ...
    async function renderStats() {
      ...
      try {
        ...
        showToast(e.message || "Failed to load statistics", "error");
      }
    ...
    async function renderCurrent() {
      try {
        ...
        console.error("Error syncing backend in renderCurrent:", err);
      }
      populateSelects();
      const activePanel = container.querySelector('.pv-panel.active')?.id;
      if (activePanel === 'pv-pay')      await renderUnpaid();
      if (activePanel === 'pv-receipts') await renderReceipts();
      if (activePanel === 'pv-stats' && currentUser.role !== 'user')    await renderStats();
    }

    /* ── filter events ── */
    q('#pv-pay-period').addEventListener('change', async () => await renderUnpaid());
    q('#pv-pay-hh').addEventListener('change', async () => await renderUnpaid());
    q('#pv-rec-filter-btn').addEventListener('click', async () => await renderReceipts());
    q('#pv-rec-clear-btn').addEventListener('click', async () => {
      q('#pv-rec-hh').value = ''; q('#pv-rec-from').value = ''; q('#pv-rec-to').value = '';
      await renderReceipts();
    });
    if (q('#pv-year-sel')) q('#pv-year-sel').addEventListener('change', async () => await renderStats());
