 ...
 * Quản lý thu phí hộ gia đình — tích hợp với REST APIs của Spring Boot
 * và cơ sở dữ liệu MySQL làm nguồn dữ liệu duy nhất.
 *
...
const FM = {
  db: { fees: [], periods: [], households: [], assignedFees: [], utilityRecords: [] },

  async syncWithBackend(selectedPeriodId) {
    const [fees, periods, households, utilityRecords] = await Promise.all([
      API.fetchJson('/fees'),
      API.fetchJson('/collection-periods'),
      API.fetchJson('/residents/households').then(page => page.content || []),
      API.fetchJson('/utility-records')
    ]);

    this.db = { fees, periods, households, assignedFees: [], utilityRecords };

    let pId = selectedPeriodId;
    if (!pId && periods.length > 0) {
      pId = periods[0].id;
    }

    if (pId) {
      const assigned = await API.fetchJson(`/payments/by-period/${pId}`).then(page => page.content || []);
      this.db.assignedFees = assigned;
    }
  ...
  async createFee(name, type, calcMethod, price) {
    await API.fetchJson('/fees', {
      method: 'POST',
      body: JSON.stringify({ name, type, calcMethod, price })
    });
  },

  async updateFee(id, name, type, calcMethod, price) {
    await API.fetchJson(`/fees/${id}`, {
      method: 'PUT',
      body: JSON.stringify({ name, type, calcMethod, price })
    });
  },
  ...
  // ----- PERIODS -----
  getPeriods() { return this.db.periods; },

  async createPeriod(name, feeIds) {
    return await API.fetchJson('/collection-periods', {
      method: 'POST',
  ...
  async closePeriod(id) {
    await API.fetchJson(`/collection-periods/${id}/close`, {
      method: 'PUT'
    });
  },

  // ----- HOUSEHOLDS -----
  getHouseholds() { return this.db.households; },

  async createHousehold(id, ownerName, membersCount, area, motorcycleCount, carCount) {
    await API.fetchJson('/residents/households', {
      method: 'POST',
      body: JSON.stringify({ id, ownerName, membersCount, area, motorcycleCount, carCount, status: 'OCCUPIED' })
    });
  },

  // ----- ASSIGN -----
  async assignFee(householdId, periodId, feeId, quantity) {
    await API.fetchJson('/payments/assigned-fees', {
      method: 'POST',
      body: JSON.stringify({ householdId, periodId, feeId, quantity })
    });
  },

  async unassignFee(householdId, periodId, feeId) {
    await API.fetchJson(`/payments/assigned-fees/${householdId}/${periodId}/${feeId}`, {
      method: 'DELETE'
    });
  },

  async payFee(assignedFeeId) {
    // Handled on backend
  },

  async unpayFee(assignedFeeId) {
    await API.fetchJson(`/payments/${assignedFeeId}`, {
      method: 'DELETE'
    });
  },

  async updateUtilityIndex(householdId, periodId, feeId, oldIndex, newIndex) {
    await API.fetchJson('/utility-records', {
      method: 'POST',
      body: JSON.stringify({ householdId, periodId, feeId, oldIndex, newIndex })
    });
  },
  ...
  calcBill(householdId, periodId) {
    ...
    for (const af of afs) {
      ...
      if (!fee) continue;
      const amount = af.amountRequired || (fee.price * af.quantity);
...
export class FeeManagerView {
  static render(container, showToast, user) {
      ...
      <div class="sf-wrap">
        <div class="chart-card" style="margin-bottom:20px;padding:16px 20px;display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:12px;">
          <div>
            ...
            <p class="card-title-muted">Single source of truth via Spring Boot backend APIs and MySQL database</p>
          </div>
        </div>
    ...
    async function renderAll() {
      try {
        selectedPeriodId = await FM.syncWithBackend(selectedPeriodId);
        refreshPeriodSel();
        renderDashboard();
        renderFees();
    ...
    function openBillModal(hhId) {
      ...
      q('#sf-bill-tbody').querySelectorAll('.sf-index-old, .sf-index-new').forEach(inp => {
        inp.addEventListener('change', async () => {
          ...
          try {
            await FM.updateUtilityIndex(hhId, selectedPeriodId, fid, oldVal, newVal);
            await renderAll(); openBillModal(hhId); showToast('Updated consumption indexes', 'success');
          } catch (err) {
            showToast(err.message, 'error');
          }
        });
      ...
      q('#sf-bill-tbody').querySelectorAll('.sf-pay').forEach(b => b.addEventListener('click', async () => {
        try {
          ...
          await renderAll(); openBillModal(hhId);
        } catch (err) {
          showToast(err.message, 'error');
        }
      }));
      q('#sf-bill-tbody').querySelectorAll('.sf-unpay').forEach(b => b.addEventListener('click', async () => {
        try {
          await FM.unpayFee(b.dataset.id);
      ...
      q('#sf-bill-tbody').querySelectorAll('.sf-unassign').forEach(b => b.addEventListener('click', async () => {
        ...
        try {
          await FM.unassignFee(hhId, selectedPeriodId, b.dataset.fid);
          showToast('Fee unassigned', 'info');
          await renderAll(); openBillModal(hhId);
        } catch (err) {
          showToast(err.message, 'error');
        }
    ...
    if (feeForm) {
      feeForm.addEventListener('submit', async e => {
        ...
        try {
          if (id) { await FM.updateFee(id, name, type, calc, price); showToast('Fee updated', 'success'); }
          else { await FM.createFee(name, type, calc, price); showToast('Fee created', 'success'); }
          close('sf-ov-fee'); await renderAll();
        } catch (err) {
          showToast(err.message, 'error');
        }
      });
    ...
    if (hhForm) {
      hhForm.addEventListener('submit', async e => {
        ...
        try {
          ...
          showToast('Household added', 'success'); close('sf-ov-hh'); await renderAll();
        } catch (err) {
          showToast(err.message, 'error');
        }
      });
    ...
    if (formPeriod) {
      formPeriod.addEventListener('submit', async e => {
        ...
        try {
          ...
          q('#sf-pname').value = '';
          showToast('Collection period created', 'success'); await renderAll();
        } catch (err) {
          showToast(err.message, 'error');
        }
      });
    ...
    if (assignForm) {
      assignForm.addEventListener('submit', async e => {
        ...
        try {
          await FM.assignFee(hhId, selectedPeriodId, feeId, Number(q('#sf-assign-qty').value));
          showToast('Fee assigned', 'success'); await renderAll(); openBillModal(hhId);
        } catch (err) {
          showToast(err.message, 'error');
        }
      });
